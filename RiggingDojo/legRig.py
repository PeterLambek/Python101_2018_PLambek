...
def  L_fk_leg():
    ...
    cmds.xform(fkFemur, t=posTransFemur, ws=True)
    cmds.xform(fkKnee, t=posTransKnee, ws=True)
    cmds.xform(fkAnkle, t=posTransAnkle, ws=True)
    cmds.xform(fkBall, t=posTransBall, ws=True)
...
cmds.parent(ik_foot_ctl, ik_grp)
# freeze ik ctl's
...
cmds.parent(gimbal_grp, "L_Ik_offsetFoot_Ctl")
...
cmds.shadingNode('multiplyDivide', n='L_ballRevFoot_foorRoll_Md' au=True)
cmds.setAttr('L_ballRevFoot_foorRoll_Md.operation', 1)
# toe
cmds.shadingNode('setRange', n='L_tipRevFoot_Sr', au=True)
cmds.shadingNode('multiplyDivide', n='L_tipRoll_foorRoll_Md', au=True)
...
cmds.orientConstraint(ik_foot_ctl, ik_ankle, mo=True)
