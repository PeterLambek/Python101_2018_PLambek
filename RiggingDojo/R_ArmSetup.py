...
def R_Ik_armStretch():
    ...
    armDist = cmds.rename("distanceDimension1", "R_Ik_shldrWrist_Dist")
    shldrDist = cmds.rename("locator1", "R_Ik_shldrDist_Loc")
    wristDist = cmds.rename("locator2", "R_Ik_wristDist_Loc")

    # place them in a group and place them in the hierarchy

    distGrp = cmds.group(armDist, shldrDist, wristDist, n="R_Ik_distanceSetup_Grp")

    # parentConstrint the locators to the joints

    cmds.parentConstraint('R_Ik_shoulder_jnt', 'R_Ik_shldrDist_Loc', maintainOffset=False)
    cmds.parentConstraint('R_Ik_wrist_jnt', 'R_Ik_wristDist_Loc', maintainOffset=False)

    # place the group in the main hierarchy

    cmds.parent(distGrp, "R_Ik_Grp")

    # create the nodes needed for the stretch

    R_remapV = cmds.shadingNode('remapValue', asUtility=True, n="R_Ik_arm_sS_RemapValue")
    R_sS_Md = cmds.shadingNode('multiplyDivide', asUtility=True, n="R_sS_Md")
    R_sS_Clamp = cmds.shadingNode('clamp', asUtility=True, n="R_sS_Clamp")
    R_sS_onOff_Cnd = cmds.shadingNode('condition', asUtility=True, n="R_sS_ONOFF_Cnd")
    R_SS_Cnd = cmds.shadingNode('condition', asUtility=True, n="R_SS_Cnd")
    R_offPost_Md =cmds.shadingNode('multiplyDivide', asUtility=True, n="R_offsetPost_Md")
    R_offPre_Md = cmds.shadingNode('multiplyDivide', asUtility=True, n="R_offsetPre_Md")
    R_wrist_transAbs_Md = cmds.shadingNode('multiplyDivide', asUtility=True, n="R_wrist_transAbs_Md")
    R_elbow_transAbs_Md = cmds.shadingNode('multiplyDivide', asUtility=True, n="R_elbow_transAbs_Md")
    R_postMid_Bta = cmds.shadingNode('blendTwoAttr', asUtility=True, n="R_postMid_Bta")
    R_preMid_Bta = cmds.shadingNode('blendTwoAttr', asUtility=True, n="R_preMid_Bta")

    # create the attributes on the ik hand control

    cmds.addAttr('R_Ik_arm_Ctl', longName=STRETCH, edit=True, attributeType='enum')

    # connect the nodes
    # connecting the distanceDim to the multiplyDivide, and set the attributes

    cmds.connectAttr(armDist+'.distance', R_sS_Md+'.input1X')
...
def R_rig_arm():   
    ...
    if len(R_arm_sel) == 5:
        ...
        clavicle = cmds.rename(R_arm_sel[0], 'R_Rig_clavicle_jnt')

        shoulder = cmds.rename(R_arm_sel[1], 'R_Rig_shoulder_jnt')

        elbow = cmds.rename(R_arm_sel[2], 'R_Rig_elbow_jnt')

        wrist = cmds.rename(R_arm_sel[3], 'R_Rig_wrist_jnt')

        wristEnd = cmds.rename(R_arm_sel[4], 'R_Rig_wristEnd')
    ...
    if placeInGroup:
        ...
        cmds.parent('R_Rig_clavicle_jnt', 'skeleton_Grp')
...
def R_rig_arm_mirror():
    ...
    cmds.mirrorJoint('R_Rig_clavicle_jnt', mirrorYZ=True, mirrorBehavior=True, searchReplace=("R_", "R_"))
...
def R_fk_arm():   
    ...
    fk_arm = cmds.duplicate('R_Rig_clavicle_jnt', renameChildren=True)   

    cmds.listRelatives(fk_arm, allDescendents=True)   

    fkClavicle = cmds.rename(fk_arm[0], 'R_Fk_clavicle_jnt')
    fkShoulder = cmds.rename(fk_arm[1], 'R_Fk_shoulder_jnt')
    fkElbow = cmds.rename(fk_arm[2], 'R_Fk_elbow_jnt')
    fkWrist = cmds.rename(fk_arm[3], 'R_Fk_wrist_jnt')
    fkWristEnd = cmds.rename(fk_arm[4], 'R_Fk_wristEnd_jnt')   
    ...
    posOrientShoulder = cmds.xform('R_Fk_shoulder_jnt', q=True, ro=True, ws=True)
    posOrientElbow = cmds.xform('R_Fk_elbow_jnt', q=True, ro=True, ws=True)
    posOrientWrist = cmds.xform('R_Fk_wrist_jnt', q=True, ro=True, ws=True)
    ...
    cmds.xform('R_Fk_shoulder_Ctl', ro=(0, 90, 0), ws=True)
    cmds.xform('R_Fk_elbow_Ctl', ro=(0, 90, 0), ws=True)
    cmds.xform('R_Fk_wrist_Ctl', ro=(0, 90, 0), ws=True)
   ...
   # orient the group after the respected name
    ...
    cmds.makeIdentity('R_Fk_shoulder_Ctl', a=True)
    cmds.makeIdentity('R_Fk_elbow_Ctl', a=True)
    cmds.makeIdentity('R_Fk_wrist_Ctl', a=True)
   ...
   # delete history of the controllers
    ...
    cmds.setAttr('R_Fk_shoulder_Ctl.translateX', lock=True, channelBox=False, keyable=False)
    cmds.setAttr('R_Fk_shoulder_Ctl.translateY', lock=True, channelBox=False, keyable=False)
    cmds.setAttr('R_Fk_shoulder_Ctl.translateZ', lock=True, channelBox=False, keyable=False)
    cmds.setAttr('R_Fk_shoulder_Ctl.scaleX', lock=True, channelBox=False, keyable=False)
    cmds.setAttr('R_Fk_shoulder_Ctl.scaleY', lock=True, channelBox=False, keyable=False)
    cmds.setAttr('R_Fk_shoulder_Ctl.scaleZ', lock=True, channelBox=False, keyable=False)
    cmds.setAttr('R_Fk_shoulder_Ctl.visibility', lock=True, channelBox=False, keyable=False)

    # Elbow

    cmds.setAttr('R_Fk_elbow_Ctl.translateX', lock=True, channelBox=False, keyable=False)
    cmds.setAttr('R_Fk_elbow_Ctl.translateY', lock=True, channelBox=False, keyable=False)
    cmds.setAttr('R_Fk_elbow_Ctl.translateZ', lock=True, channelBox=False, keyable=False)
    cmds.setAttr('R_Fk_elbow_Ctl.scaleX', lock=True, channelBox=False, keyable=False)
    cmds.setAttr('R_Fk_elbow_Ctl.scaleY', lock=True, channelBox=False, keyable=False)
    cmds.setAttr('R_Fk_elbow_Ctl.scaleZ', lock=True, channelBox=False, keyable=False)
    cmds.setAttr('R_Fk_elbow_Ctl.visibility', lock=True, channelBox=False, keyable=False)

    # Wrist

    cmds.setAttr('R_Fk_wrist_Ctl.translateX', lock=True, channelBox=False, keyable=False)
    cmds.setAttr('R_Fk_wrist_Ctl.translateY', lock=True, channelBox=False, keyable=False)
    cmds.setAttr('R_Fk_wrist_Ctl.translateZ', lock=True, channelBox=False, keyable=False)
    cmds.setAttr('R_Fk_wrist_Ctl.scaleX', lock=True, channelBox=False, keyable=False)
    cmds.setAttr('R_Fk_wrist_Ctl.scaleY', lock=True, channelBox=False, keyable=False)
    cmds.setAttr('R_Fk_wrist_Ctl.scaleZ', lock=True, channelBox=False, keyable=False)
    cmds.setAttr('R_Fk_wrist_Ctl.visibility', lock=True, channelBox=False, keyable=False)
    ...
    if placeInGroup:
        cmds.parent('R_Fk_shoulder_Grp', 'R_Fk_Grp')       
...
def R_ik_arm():
    ...
    ik_arm = cmds.duplicate('R_Rig_clavicle_jnt', renameChildren=True)   

    cmds.listRelatives(ik_arm, allDescendents=True)   

    ikClavicle = cmds.rename(ik_arm[0], 'R_Ik_clavicle_jnt')
    ikShoulder = cmds.rename(ik_arm[1], 'R_Ik_shoulder_jnt')
    ikElbow = cmds.rename(ik_arm[2], 'R_Ik_elbow_jnt')
    ikWrist = cmds.rename(ik_arm[3], 'R_Ik_wrist_jnt')
    ikWristEnd = cmds.rename(ik_arm[4], 'R_Ik_wristEnd_jnt')      
    ...
    # create the control
    lIkControl = cmds.circle(n='R_Ik_arm_Ctl', nr=(0, 0, 1), c=(0, 0, 0), r=0.35)
    # orient the control to the wrist
    cmds.xform('R_Ik_arm_Ctl', ro=(0, 90, 0), ws=True)
    ...
    # freeze history and clear history of control
    cmds.makeIdentity('R_Ik_arm_Ctl', a=True)
    cmds.delete('R_Ik_arm_Ctl', ch=True)
    ...
    cmds.spaceLocator(n='R_arm_poleVec_Loc')
    # create a group as the group for a poleVector
    cmds.group(em=True, n='R_arm_poleVec_Grp')
    # parent locator to the group
    cmds.parent('R_arm_poleVec_Loc', 'R_arm_poleVec_Grp')
    # place the group between the shoulder and the wrist
    cmds.pointConstraint('R_Ik_shoulder_jnt', 'R_Ik_wrist_jnt', 'R_arm_poleVec_Grp')
    # aim the locator twoards the elbow
    cmds.aimConstraint('R_Ik_elbow_jnt', 'R_arm_poleVec_Grp', aim=(1, 0, 0), u=(0, 1, 0))
    # delete the constraints
    cmds.delete('R_arm_poleVec_Grp_pointConstraint1')
    cmds.delete('R_arm_poleVec_Grp_aimConstraint1')
    # place the locater out from the elbow using the X axis trans
    cmds.move(25, 'R_arm_poleVec_Loc', x=True, os=True, ws=False)
    # create controller for the polevector
    cmds.curve(n='R_Ik_elbow_Ctl', d = 1,p = [[0.5000000000000019, -0.250000000000001, 5.551115123125783e-17], [0.5000000000000019, 0.249999999999999, -5.551115123125783e-17],
    ...
    cmds.scale(0.8, 0.8, 0.8, 'R_Ik_elbow_Ctl')
    # move parent the controller to the locator locatieon
    cmds.pointConstraint('R_arm_poleVec_Loc', 'R_Ik_elbow_Ctl')
    # delete pointConstraint from controller
    cmds.delete('R_Ik_elbow_Ctl_pointConstraint1')
    # parent controller to grp
    cmds.parent('R_Ik_elbow_Ctl', 'R_arm_poleVec_Grp')
    # freeze orientation on controller
    cmds.makeIdentity('R_Ik_elbow_Ctl', a=True)
    # parent poleVEc to controller
    cmds.parent('R_arm_poleVec_Loc', 'R_Ik_elbow_Ctl')
    # connect the polevector constraint to the ikhandle and the locator
    cmds.poleVectorConstraint('R_arm_poleVec_Loc', 'R_arm_Ikh')
    # hide locator
    cmds.hide('R_arm_poleVec_Loc')
    # lock and hide scale and visibility
    # ik hand ctl
    cmds.setAttr('R_Ik_arm_Ctl.scaleX', lock=True, channelBox=False, keyable=False)
    cmds.setAttr('R_Ik_arm_Ctl.scaleY', lock=True, channelBox=False, keyable=False)
    cmds.setAttr('R_Ik_arm_Ctl.scaleZ', lock=True, channelBox=False, keyable=False)
    cmds.setAttr('R_Ik_arm_Ctl.visibility', lock=False, channelBox=False, keyable=False)
    # Elbow ik ctl
    cmds.setAttr('R_Ik_elbow_Ctl.scaleX', lock=True, channelBox=False, keyable=False)
    cmds.setAttr('R_Ik_elbow_Ctl.scaleY', lock=True, channelBox=False, keyable=False)
    cmds.setAttr('R_Ik_elbow_Ctl.scaleZ', lock=True, channelBox=False, keyable=False)
    cmds.setAttr('R_Ik_elbow_Ctl.visibility', lock=False, channelBox=False, keyable=False)
    # place them in the group hierarchy if there is one
    placeInGroup = cmds.ls('main_Grp')
    # if it is there parent them respectly
    if placeInGroup:
        cmds.parent('R_Ik_arm_Grp', 'R_Ik_Grp')
        cmds.parent('R_arm_poleVec_Grp', 'R_Ik_Grp')
...
def R_connectFk_armSetup():
    ...
    if createCtl:
        ...
        cmds.circle(n='R_arm_IkFkSwitch_Ctl', nr=(0, 0, 1), c=(0, 0, 0), r=0.75)
        # create a group for the switch control
        cmds.group(em=True, n='R_arm_IkFkSwitch_Grp')
        # parent the switch controller to the group
        cmds.parent('R_arm_IkFkSwitch_Ctl', 'R_arm_IkFkSwitch_Grp')
        # place the group above the hand
        cmds.xform('R_arm_IkFkSwitch_Grp', t=posTransWrist, ws=True)
        cmds.xform('R_arm_IkFkSwitch_Grp', t=posOrientWrist, ws=True)
        # turn the control to be horizontal and move it up
        cmds.xform('R_arm_IkFkSwitch_Ctl', r=True, ro=(-90, 0, 0))
        cmds.xform('R_arm_IkFkSwitch_Ctl', r=True, t=(0, 2, 0))
        # freeze the controlle
        cmds.makeIdentity('R_arm_IkFkSwitch_Ctl', a=True)
        # remove history
        cmds.delete('R_arm_IkFkSwitch_Ctl', ch=True) 
        # create attributes on the controller for ik fk switch, and the visibility
        cmds.addAttr('R_arm_IkFkSwitch_Ctl', ln="ikFkSwitch", at='enum', en='Ik:Fk:', k=True)
        cmds.addAttr('R_arm_IkFkSwitch_Ctl', ln="ikFkVisibility", at='enum', en='auto:both:', k=True)
        # connect the pairBlend to the ikFkSwitch
        cmds.connectAttr('R_arm_IkFkSwitch_Ctl.ikFkSwitch', 'R_shoulder_Pb.weight')
        cmds.connectAttr('R_arm_IkFkSwitch_Ctl.ikFkSwitch', 'R_elbow_Pb.weight')
        cmds.connectAttr('R_arm_IkFkSwitch_Ctl.ikFkSwitch', 'R_wrist_Pb.weight')
        # hide controls based on the selection ikFkVisibility attribute
        # create condition node and two reverse nodes for the setup
        cmds.shadingNode('reverse', n="R_arm_ikFkVisibility01_Rev", au=True)
        cmds.shadingNode('reverse', n="R_arm_ikFkVisibility02_Rev", au=True)
        cmds.shadingNode('condition', n="R_arm_ikFkVisibility_Cnd", au=True)
        # set attributes on the condition node
        cmds.setAttr('R_arm_ikFkVisibility_Cnd.operation', 0)
        cmds.setAttr('R_arm_ikFkVisibility_Cnd.colorIfTrueR', 1)
        cmds.setAttr('R_arm_ikFkVisibility_Cnd.colorIfTrueG', 1)
        # connect the ikFkSwitch to the nodes
        cmds.connectAttr('R_arm_IkFkSwitch_Ctl.ikFkSwitch', "R_arm_ikFkVisibility01_Rev.input.inputX")
        cmds.connectAttr('R_arm_IkFkSwitch_Ctl.ikFkSwitch', "R_arm_ikFkVisibility_Cnd.colorIfFalseR")
        cmds.connectAttr('R_arm_IkFkSwitch_Ctl.ikFkVisibility', "R_arm_ikFkVisibility02_Rev.input.inputX")
        # connect the reverse nodes to the condition
        cmds.connectAttr("R_arm_ikFkVisibility01_Rev.output.outputX", "R_arm_ikFkVisibility_Cnd.colorIfFalseG")
        cmds.connectAttr("R_arm_ikFkVisibility02_Rev.output.outputX", "R_arm_ikFkVisibility_Cnd.firstTerm")
        # connect the condition node to ik and fk control top groups, outColorG = Ik outColorR = Fk
        cmds.connectAttr('R_arm_ikFkVisibility_Cnd.outColorG', 'R_Ik_arm_Grp.visibility')
        cmds.connectAttr('R_arm_ikFkVisibility_Cnd.outColorG', 'R_arm_poleVec_Grp.visibility')
        cmds.connectAttr('R_arm_ikFkVisibility_Cnd.outColorG', 'R_Ik_shoulder_jnt.visibility')
        cmds.connectAttr('R_arm_ikFkVisibility_Cnd.outColorR', 'R_Fk_shoulder_Grp.visibility')
        cmds.connectAttr('R_arm_ikFkVisibility_Cnd.outColorR', 'R_Fk_shoulder_jnt.visibility')
        # lock and hide the translate, rotate, scale and hide visibility
        #translate
        cmds.setAttr('R_arm_IkFkSwitch_Ctl.translateX', lock=True, channelBox=False, keyable=False)
        cmds.setAttr('R_arm_IkFkSwitch_Ctl.translateY', lock=True, channelBox=False, keyable=False)
        cmds.setAttr('R_arm_IkFkSwitch_Ctl.translateZ', lock=True, channelBox=False, keyable=False)
        #rotate
        cmds.setAttr('R_arm_IkFkSwitch_Ctl.rotateX', lock=True, channelBox=False, keyable=False)
        cmds.setAttr('R_arm_IkFkSwitch_Ctl.rotateY', lock=True, channelBox=False, keyable=False)
        cmds.setAttr('R_arm_IkFkSwitch_Ctl.rotateZ', lock=True, channelBox=False, keyable=False)
        #scale
        cmds.setAttr('R_arm_IkFkSwitch_Ctl.scaleX', lock=True, channelBox=False, keyable=False)
        cmds.setAttr('R_arm_IkFkSwitch_Ctl.scaleY', lock=True, channelBox=False, keyable=False)
        cmds.setAttr('R_arm_IkFkSwitch_Ctl.scaleZ', lock=True, channelBox=False, keyable=False)
        #visibility
        cmds.setAttr('R_arm_IkFkSwitch_Ctl.visibility', lock=False, channelBox=False, keyable=False)
        # parent the group respectively
        cmds.parent('R_arm_IkFkSwitch_Grp', 'R_Ik_Grp')
        # get the control to follow the hand by getting the top group parentConstrainted
        cmds.parentConstraint('R_Rig_wrist_jnt', 'R_arm_IkFkSwitch_Grp', maintainOffset=True)
...
def R_connectIk_armSetup():
    ...
    if createCtl:
        ...
        cmds.connectAttr('R_ikFkVisibility_Cnd.outColorG', 'R_arm_poleVec_Grp.visibility')
        ...
        cmds.parent('R_IkFkSwitch_Grp', 'R_Ik_Grp')
...
def R_connectFk_armSetup():
    ...
    createCtl = cmds.ls('R_arm_IkFkSwitch_Ctl')
...
def R_connectIk_armSetup():
    ...
    if createCtl:
        ...
        cmds.connectAttr('R_arm_ikFkVisibility_Cnd.outColorG', 'R_poleVec_Grp.visibility')
